...
namespace Verse.AI
{
	public static class AttackTargetFinder
	{
		...
		public static Thing BestAttackTarget(Thing searcher, TargetScanFlags flags, Predicate<Thing> validator = null, float minDist = 0f, float maxDist = 9999f, IntVec3 locus = default(IntVec3), float maxTravelRadiusFromLocus = 3.40282347E+38f, bool canBash = false)
		{
			...
			if (AttackTargetFinder.HasRangedAttack(searcher))
			{
				...
				Thing result = GenClosest.ClosestThing_Global(searcher.Position, AttackTargetFinder.tmpTargets, maxDist, predicate);
				AttackTargetFinder.tmpTargets.Clear();
				return result;
			}
